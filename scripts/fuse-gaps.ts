...
const git = (...args: string[]) =>
  execFileSync('git', args, { cwd: ROOT, maxBuffer: 1 << 28, stdio: ['ignore', 'pipe', 'ignore'] }).toString()
...
function attemptUniverse(): Universe {
  const headDetected = gitJson<Record<string, FuseDetection>>('HEAD', 'data/fuses-detected.json') ?? {}
  const uncommitted = Object.keys(detected).some((id) => !headDetected[id])
  if (uncommitted) {
    return { ids: new Set(videos.map((v) => v.id)), commit: '(working tree)', runDate: new Date().toISOString() }
  }
  const commits = git('log', '--format=%H', '--', 'data/fuses-detected.json').trim().split('\n').filter(Boolean)
  for (const c of commits) {
    ...
    const snapshot = gitJson<VideoRecord[]>(`${c}~1`, 'data/videos.json') ?? gitJson<VideoRecord[]>(c, 'data/videos.json') ?? []
    return {
...
const counts = Object.fromEntries(
  (['unavailable', 'low', 'none', 'pending', 'anomaly'] as const).map((b) => [b, inBucket(b).length]),
...
const lowIds = Object.entries(detected).filter(([, d]) => d.status === 'low').map(([id]) => id)
...
if (existsSync(sheetPath)) {
  const sheetIds = [...readFileSync(sheetPath, 'utf8').matchAll(/^\|\s*([A-Za-z0-9_-]{11})\s*\|/gm)].map((m) => m[1]!)
  const sheetSet = new Set(sheetIds)
  const stale = sheetIds.filter((id) => detected[id]?.status !== 'low' && detected[id]?.status !== 'none')
    ...
    for (const f of picks) {
      ...
      const crop = await sharp(frame)
        .extract({
          ...
          height: Math.min(Math.round((rh + PAD_TOP + PAD_BOTTOM) * meta.height), meta.height - top),
        })
...
md.push('## Summary')
md.push('')
md.push(`${withFuse} of ${videos.length} videos have a fuse — **${missing.length} missing**, split:`)
md.push('')
md.push('| bucket | count | meaning | action |')
md.push('|---|---|---|---|')
md.push(`| 1 · UNAVAILABLE | **${counts.unavailable}** | attempted by the last run, download failed — no frames were ever cached | see the still-listed note below — most look retryable, not permanently lost |`)
md.push(`| 2 · LOW-CONFIDENCE | **${counts.low}** | pill seen, best guess under threshold | adjudicate via gap-pills.png → promote through overrides.json |`)
md.push(`| 3 · NONE | **${counts.none}** | frames cached, no saturated pill in any frame | ${counts.none ? 'eyeball for occlusion / new template class' : '— (none exist right now)'} |`)
md.push(`| 4 · NOT-YET-PROCESSED | **${counts.pending}** | added by the daily cron after the last run | picked up by the next \`data:fuses\` |`)
md.push(`| ⚠ ANOMALY | **${counts.anomaly}** | confident detection exists but could not merge | fix the underlying record (see below) |`)
md.push('')

md.push('## Reconciliation facts')
md.push('')
md.push(`- **ok-unordered vs truly absent:** all ${statusTally['ok-unordered'] ?? 0} ok-unordered detections merged into videos.json (they count as “has fuse”, shown unattributed). Among the ${missing.length} missing, **${unmergedConfident.length}** carr${unmergedConfident.length === 1 ? 'ies' : 'y'} a confident detection that could not merge${unmergedConfident.length ? `: ${unmergedConfident.map((i) => `\`${i.id}\` (status ${i.detection?.status}, video has ${byId.get(i.id)?.teams.length ?? 0} parsed teams — needs a title-parse override, not fuse work)`).join('; ')}` : ''}. Every other miss is genuinely fuse-less.`)
md.push(`- **UNAVAILABLE videos are still listed on YouTube.** \`data:fetch\` re-walks the full uploads playlists daily and drops ids the API stops returning (deleted/private), and videos.json is regenerated from that dump — so surviving today's refresh means YouTube still lists them. yt-dlp failing on 2026-07-02 while the API still serves them points at run-time throttling / age- or region-gating, **not** deletions. Orphan check agrees: ${orphanDetections.length} detected ids have dropped out of videos.json.`)
md.push(`- **Overrides already rescued ${lowRescued.length} low ids** (${lowRescued.map((id) => `\`${id}\``).join(', ') || '—'}) — ${lowIds.length} low total → ${counts.low} still missing. No override targets a missing video without giving it a fuse.`)
...
if (reviewables.length) {
  md.push(`${s0beta}/${reviewables.length} (${Math.round((100 * s0beta) / reviewables.length)}%) sit in beta/s0 footage — the same old-capture cluster as the confidence drift.`)
  md.push('')
}
...
md.push('')
md.push(`Pill crops for every row are in **gap-pills.png** (${montageRows} rows; #NNN matches the montage label, L/R appended per side). Guesses/distances are the raw reads from fuses-detected.json — left/right are in title order.`)
md.push('')
...
md.push('')
if (counts.none === 0) {
  md.push('Empty today: the 2026-07-03 re-detect left zero `none` records — every video with frames read *something* off the pill window. If a future run produces them, they land in this section and in gap-pills.png, and are the place to hunt for missing template classes (sidekick still has zero specimens).')
} else {
  md.push('| montage | video | era | channel | frames |')
  md.push('|---|---|---|---|---|')
  reviewables.forEach((i, idx) => {
    if (i.bucket !== 'none') return
    md.push(`| #${String(idx + 1).padStart(3, '0')} | \`${i.id}\` ${yt(i.id)} | ${i.era} | ${i.channel} | cache/fuse/frames/${i.id}/ (${i.frames}) |`)
  })
}
md.push('')
...
for (const i of inBucket('pending')) {
  const v = byId.get(i.id)!
  md.push(`| \`${i.id}\` ${yt(i.id)} | ${i.publishedAt.slice(0, 10)} | ${i.era} | ${i.channel} | ${esc(short(v.title))} |`)
}
md.push('')
...
console.log(`  unavailable  ${String(counts.unavailable).padStart(4)}  (${maybePending} flagged maybe-pending)`)
...
console.log(`✓ cache/fuse/review/fuse-gaps.md · fuse-gaps.json · gap-pills.png (${montageRows} rows)`)
