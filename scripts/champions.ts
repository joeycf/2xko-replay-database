...
// For each champion in data/champions.json:
//   • accent   ← design tokens (design/handoff/tokens.css --champ-<id>)  [source of truth]
//   • portrait ← 2xko.riotgames.com roster index (characterCardGrid, 780x1040)
//   • splash   ← champion detail page metaImage (1920x1080)
...
async function saveWebp(srcUrl: string, outPath: string, maxWidth: number): Promise<void> {
  ...
  await sharp(buf).resize({ width: maxWidth, withoutEnlargement: true }).webp({ quality: 80, effort: 4 }).toFile(outPath)
}

function loadAccents(css: string): Map<string, string> {
  const map = new Map<string, string>()
  for (const m of css.matchAll(/--champ-([a-z0-9]+):\s*(#[0-9a-fA-F]{6})/g)) map.set(m[1], m[2].toUpperCase())
  return map
}
...
async function loadPortraitRefs(): Promise<Map<string, PortraitRef>> {
  const grid = blades(nextData(await fetchText(`${SITE}/`))).find((b) => b.type === 'characterCardGrid')
...
async function main(): Promise<void> {
  ...
  const champions = JSON.parse(await readFile(join(DATA, 'champions.json'), 'utf8')) as Record<string, Champion>
  ...
  for (const id of ids) {
    ...
    if (await exists(out)) {
      champions[id].portrait = `/img/champions/${id}-portrait.webp`
      portraitOk++
    }
  }
  ...
  //    `<id>-splash-800.webp`; champions.json keeps pointing at the 1600w).
  let splashOk = 0
  for (const id of ids) {
    ...
    if (await exists(out)) {
      champions[id].splash = `/img/champions/${id}-splash.webp`
      splashOk++
    }
  }

  await writeFile(join(DATA, 'champions.json'), JSON.stringify(champions, null, 2) + '\n', 'utf8')
  console.log(
