...
const MIN_SAT_FRAC = 0.1; // pill present: saturated fraction floor (dark occlusions sit below)
...
const NAME_H = 0.028;
const NAME_Y_STEPS = [
  0.068, 0.073, 0.078, 0.083, 0.088, 0.093, 0.098, 0.103, 0.108, 0.113, 0.118, 0.123,
];
...
async function ensureFrames(id: string): Promise<string[] | null> {
  ...
  try {
    execFileSync(
      'ffmpeg',
      ['-hide_banner', '-loglevel', 'error', '-i', raw, '-vf', 'fps=1', join(dir, '%02d.png')],
      {
        env: process.env,
        timeout: 120_000,
      },
...
async function detect(
  ...
  const [L, R] = await Promise.all([
    readSide(frames, 'left', pills),
    readSide(frames, 'right', pills),
  ]);
  ...
  if (conf(L) && conf(R)) {
    if (L.fuse === R.fuse) det.status = 'ok';
      ...
      if (sameOrder === null) det.status = 'ok-unordered';
        ...
        det.status = 'ok';
        if (!sameOrder)
          det = { ...det, left: R.fuse, right: L.fuse, score: { left: R.dist, right: L.dist } };
      }
    }
...
async function runValidation(pills: PillTemplate[], names: Map<string, bigint>): Promise<void> {
  const labels = JSON.parse(
    readFileSync(join(ROOT, 'data/fuse-validation.json'), 'utf8'),
  ...
  for (const id of ids) {
    ...
    if (det.status === 'ok' || det.status === 'ok-unordered') {
        ...
        det.status === 'ok-unordered'
          ? // unordered: compare as multiset
            (() => {
              ...
              if (wantSet === gotSet) {
                bump(want.left, want.left);
                bump(want.right, want.right);
                return [];
              }
              return [
                ['left', det.left, reads.left],
                ['right', det.right, reads.right],
              ] as [string, string | null, SideRead][];
            })()
          : [
              ['left', det.left, reads.left],
              ['right', det.right, reads.right],
      ...
      for (const [side, got, read] of pairs) {
        ...
        if (got === w) {
          ...
          if (
            det.status === 'ok' &&
            [want.left, want.right].sort().join() === [det.left, det.right].sort().join()
          )
            orientWrong++;
          disagreements.push({
            id,
      ...
      disagreements.push(
        {
          ...
          want: want.left,
          got: reads.left.fuse,
          dist: reads.left.dist,
          frame: reads.left.frame,
          crop: reads.left.crop,
        },
        {
          id,
          side: 'right',
          want: want.right,
          got: reads.right.fuse,
          dist: reads.right.dist,
          frame: reads.right.frame,
          crop: reads.right.crop,
        },
      );
    }
    ...
    scoreDump[id] = {
      status: det.status,
      left: {
        fuse: reads.left.fuse,
        d: reads.left.dist,
        m: reads.left.margin,
        st: reads.left.struct,
        sat: Number(reads.left.satFrac.toFixed(3)),
      },
      right: {
        fuse: reads.right.fuse,
        d: reads.right.dist,
        m: reads.right.margin,
        st: reads.right.struct,
        sat: Number(reads.right.satFrac.toFixed(3)),
      },
      want: labels[id],
    };
  }
  ...
  console.log(
    `correct-read dist: min=${Math.min(...distsCorrect)} avg=${(distsCorrect.reduce((a, b) => a + b, 0) / distsCorrect.length).toFixed(1)} max=${Math.max(...distsCorrect)}`,
  );
  console.log(
    `margins: min=${Math.min(...margins)} avg=${(margins.reduce((a, b) => a + b, 0) / margins.length).toFixed(1)}`,
  );
  const histo = (xs: number[]) => {
    const h: Record<string, number> = {};
    for (const x of xs) {
      const b = `${Math.floor(x / 4) * 4}-${Math.floor(x / 4) * 4 + 3}`;
      h[b] = (h[b] ?? 0) + 1;
    }
  ...
  if (disagreements.length) {
    ...
    for (const d of disagreements) {
      ...
      parts.push({
        input: {
          text: {
            text: `<span foreground="#FF2E88" background="#000000"> ${d.id} ${d.side} · want ${d.want} · got ${d.got ?? '∅'} d${d.dist} </span>`,
            rgba: true,
            dpi: 110,
          },
        },
    ...
    if (y > 0) {
      await sharp({ create: { width: 1100, height: y, channels: 4, background: '#0A0B0F' } })
        .composite(parts)
        .png()
        .toFile(join(REVIEW, 'validation-disagreements.png'));
      console.log(
...
async function runBacklog(pills: PillTemplate[], names: Map<string, bigint>): Promise<void> {
  const detected: Record<string, Detection> = existsSync(OUT)
  ...
  const attempted: Record<string, string[]> = existsSync(ATTEMPTS)
    ...
    if (r) {
      ...
      if (r.det.status === 'low' || r.det.status === 'none') {
        for (const side of ['left', 'right'] as const) {
          ...
          if (read.crop && !(read.dist <= ACCEPT_DIST && read.margin >= ACCEPT_MARGIN))
            unmatched.push({ id, side, crop: read.crop, dist: read.dist });
        }
      }
    }
  ...
  if (unmatched.length) {
    ...
    await sharp({
      create: { width: cols * cell, height: rows * 60, channels: 4, background: '#0A0B0F' },
    })
      .composite(parts)
      .png()
      .toFile(join(REVIEW, 'unmatched-pills.png'));
    console.log(
      `unmatched-pill montage: cache/fuse/review/unmatched-pills.png (${unmatched.length} crops — scan for Sidekick/Teamfight clusters)`,
    );
  }
}
...
async function runPromoteLows(pills: PillTemplate[], names: Map<string, bigint>): Promise<void> {
  ...
  if (existsSync(gapsPath)) {
    const gaps = JSON.parse(readFileSync(gapsPath, 'utf8')) as {
      items: { id: string; bucket: string }[];
    };
    gaps.items
      .filter((i) => i.bucket === 'low' || i.bucket === 'none')
      .forEach((i, idx) => montageNo.set(i.id, idx + 1));
  }
  const tag = (id: string) =>
    montageNo.has(id) ? `#${String(montageNo.get(id)).padStart(3, '0')}` : '—';

  const lows = Object.entries(detected).filter(
    ([id, d]) => d.status === 'low' && !overrides[id] && byId.get(id)?.teams.length === 2,
  );
  console.log(
    `re-reading ${lows.length} low record(s) from cached frames (bar: dist ≤ ${PROMOTE_DIST} · margin ≥ ${PROMOTE_MARGIN} · struct ≤ ${STRUCT_MAX})`,
  );
  ...
  for (const [id, d] of lows) {
    ...
    let sameOrder: boolean | null =
      okL && okR && L.fuse === R.fuse ? true : await orient(frames, video, names);
    let unordered = false;
    if (sameOrder === null) {
      if (okL && okR) {
  ...
  const promotedRows = rows.filter(
    (r) => r.outcome === 'both' || r.outcome === 'single' || r.outcome === 'unordered',
  );
  const lines = [
  ...
  for (const r of promotedRows)
    console.log(`  ${tag(r.id)} ${r.id}  ${r.outcome.padEnd(9)} ${r.detail}`);
  ...
  console.log(
    `✓ orientation queue: cache/fuse/review/orient-queue.json (${orientQueue.length} — adjudicate at /dev/fuse-orient)`,
  );
}
