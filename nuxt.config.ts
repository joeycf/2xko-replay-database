...
export default defineNuxtConfig({
  ...
  extends: [process.env.ENGINE_PATH || ['github:joeycf/replay-engine#v0.11.0', { install: true }]],
  ...
  hooks: {
    ...
    // the pipeline — it builds from committed JSON, exactly like the shipped
    // videos.json flow. replays.json is the engine's whale; summary.json is the
    // apex selector's card payload, fetched same-origin through the shell's
    // /2xko rewrite (Phase 6).
    'build:before'() {
