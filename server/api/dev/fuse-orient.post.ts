...
export default defineEventHandler(async (event) => {
  ...
  if (!Array.isArray(body?.assignments)) {
    throw createError({ statusCode: 400, statusMessage: 'expected { assignments: [{ id, owner: 0|1 }] }' })
  }

  const root = process.cwd()
  const queuePath = join(root, 'cache/fuse/review/orient-queue.json')
  if (!existsSync(queuePath)) throw createError({ statusCode: 404, statusMessage: 'orient-queue.json not found' })
