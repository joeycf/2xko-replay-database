...
export default defineEventHandler(async (event) => {
  ...
  if (!id || !Array.isArray(chars) || chars.length !== 2 || !chars.every(isSide)) {
    throw createError({
      statusCode: 400,
      statusMessage: 'expected { id, characters: [string[], string[]] }',
    });
  }
  ...
  if (
    fusesIn !== undefined &&
    (!Array.isArray(fusesIn) ||
      fusesIn.length !== 2 ||
      !fusesIn.every((f) => f === null || typeof f === 'string'))
  ) {
    throw createError({
      statusCode: 400,
      statusMessage: 'expected fuses: [id|null, id|null] when provided',
    });
  }
  ...
  if (unknown.length > 0) {
    throw createError({
      statusCode: 400,
      statusMessage: `unknown champion id(s): ${unknown.join(', ')}`,
    });
  }
  if (fusePair) {
    ...
    if (badFuses.length > 0) {
      throw createError({
        statusCode: 400,
        statusMessage: `unknown fuse id(s): ${badFuses.join(', ')} (valid: ${Object.keys(fuses).sort().join(', ')})`,
      });
    }
  }
  ...
  if (!Array.isArray(entry.teams) || entry.teams.length !== 2) {
    throw createError({
      statusCode: 409,
      statusMessage: `entry "${id}" is malformed (expected 2 teams) — fix the file by hand`,
    });
  }

  const dedupe = (xs: string[]) => [...new Set(xs)];
  // the length-2 guard above doesn't narrow indexed access under
  // noUncheckedIndexedAccess — assert
  ...
  entry.teams.forEach((t, i) => {
    ...
    if (t.characters.length === 0) warnings.push(`${side} side saved with 0 champions`);
      ...
      warnings.push(
        `${side} side has an odd count (${t.characters.length}) — fine for set unions, double-check`,
      );
  });
