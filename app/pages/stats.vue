<template>
  <div class="mx-auto w-full max-w-[1440px]">
    ...
    <div class="flex flex-wrap items-center gap-x-4 gap-y-3 px-4 pb-1.5 pt-[22px] md:px-7">
      <h1 class="font-display text-[22px] font-bold text-ink-primary md:text-[26px]">
        Meta Stats
      </h1>
      <span class="font-mono text-[11px] text-ink-muted">
        {{ seasonName }} ·
        <span data-testid="context-count">
          {{ contextCount.toLocaleString('en-US') }}
        </span> replays
      </span>
      <div
      ...
      >
        <span class="mr-0.5 hidden font-sans text-[10px] font-semibold uppercase tracking-[.16em] text-ink-muted sm:block">
          Season
        </span>
        <button
          type="button"
    ...
    <div class="px-4 py-5 md:px-7">
      <StatPanel
        ...
        :hint="`appearances · ${seasonName}`"
      >
        <ChampionUsageBars :items="usageRows" />
      </StatPanel>
    </div>

    <!-- Panel 2: fuse usage (NEW vs design mockups — Phase 7 CV detections) -->
    ...
    <!-- Panel 4: meta over time + fuse meta by era -->
    <div class="grid grid-cols-1 gap-4 px-4 pb-6 md:grid-cols-2 md:px-7">
      <StatPanel
        title="Meta over time"
        :hint="`usage rank · ${eraLabel(eras[0] ?? '0')} → ${eraLabel(eras[eras.length - 1] ?? '2')}`"
      >
...
const tiles = [{
  label: 'Total replays',
  value: totals.videos.toLocaleString('en-US'),
  accent: '#FF2E88'
}, {
  label: 'Most-used champ',
  value: cname(topChamp.id),
  accent: caccent(topChamp.id)
}, {
  label: 'Top pairing',
  value: `${cname(topPair.a)} + ${cname(topPair.b)}`,
  accent: caccent(topPair.b)
}, {
  label: 'Verified pros',
  value: verifiedCount.value.toLocaleString('en-US'),
  accent: '#38CFFF'
}];

const usageRows = computed(() => usageFor(season.value));
const fuseRows = computed(() => detectedFor(season.value));
...
const fuseHint = computed(() => season.value === null
  ? `team picks · detected in ${totals.withFuse.toLocaleString('en-US')} of ${totals.videos.toLocaleString('en-US')} replays`
  : `team picks · ${seasonName.value}`
);
</script>
