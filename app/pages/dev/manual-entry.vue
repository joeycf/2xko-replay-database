<template>
  <section class="mx-auto w-full max-w-[1240px] px-4 py-8 md:px-[26px]">
    <ClientOnly>
      <p class="font-mono text-label uppercase text-text-muted">Authoring — dev only</p>
      <h1 class="mt-1 font-display text-d2 font-bold text-text">Manual entry champions</h1>
      <p class="mt-2 max-w-[760px] font-mono text-[12px] text-text-muted">
        Watch each set's VS screens and click every champion that side fielded
        <strong class="text-text">across the whole set</strong> (set-level union — any length).
        Fuses are optional — set one only when you can read the HUD pill; click again to clear.
        Saves write <span class="text-text">data/manual-videos.json</span> only; run
        <span class="text-text">npm run data:parse</span> yourself when done.
      </p>
      ...
      <div v-else-if="shown.length" class="mt-6 space-y-8 pb-28">
        ...
        >
          ...
          >
            <span
              class="border border-[rgba(245,179,60,.5)] bg-[rgba(245,179,60,.14)] px-1.5 py-0.5 text-[10px] uppercase tracking-[.1em] text-warning"
            >
              {{ e.round ?? e.tournament }}
            </span>
            <span class="text-text-muted">{{ e.tournament }}</span>
            <span class="text-text-muted">{{ e.id }}</span>
            ...
            <div>
              ...
              >
                ...
                >
                  ...
                  />
                  <span
                    class="absolute inset-0 flex items-center justify-center bg-[rgba(6,7,11,.24)]"
                  >
                    <span
                      class="flex h-14 w-14 items-center justify-center rounded-full bg-primary shadow-[0_6px_22px_rgba(255,46,136,.5)] transition-transform duration-200 group-hover:scale-105"
                    >
                      <span
                        class="ml-1 h-0 w-0 border-y-[10px] border-l-[16px] border-y-transparent border-l-white"
                      />
                    </span>
                  </span>
                </button>
              ...
              <div class="mt-2 flex items-center gap-1.5 font-mono text-[11px] text-text-muted">
                <span class="mr-1">jump:</span>
                <button
                  v-for="s in seekStops(e)"
                  :key="s.t"
                  type="button"
                  class="cursor-pointer border border-white/[0.12] bg-[#141722] px-2 py-0.5 text-text-secondary hover:text-text"
            ...
            <div class="flex flex-col gap-4">
              ...
              >
                <div class="flex items-baseline gap-2">
                  <span
                    class="font-ui text-[10px] font-semibold uppercase tracking-[.16em] text-text-muted"
                  >
                    {{ i === 0 ? 'Left' : 'Right' }}
                  </span>
                  ...
                  <span
                    ...
                    class="ml-auto font-mono text-[10px] text-text-muted"
                ...
                <div class="mt-2 flex min-h-[26px] flex-wrap items-center gap-1.5">
                  <button
                    ...
                    :key="cid"
                    type="button"
                    class="inline-flex cursor-pointer items-center gap-1.5 border border-white/[0.14] bg-[#141722] py-0.5 pl-1 pr-2 font-ui text-[12px] font-semibold text-text hover:border-danger/60"
                ...
                <div class="mt-2 flex flex-wrap gap-1.5">
                  <button
                    ...
                    :style="{
                      ...
                      borderColor: (sel[e.id]?.[i] ?? []).includes(c.id)
                        ? (c.accent ?? '#fff')
                        : 'rgba(255,255,255,.12)',
              ...
              <div class="mt-auto flex items-center gap-3">
                ...
                <span
                  v-if="cardNote[e.id]"
                  class="min-w-0 truncate font-mono text-[11px] text-text-muted"
                  >{{ cardNote[e.id] }}</span
                >
                <button
                  type="button"
...
const { list: champList, byId: champById } = useCharacters();
...
watch(
  ...
  (list) => {
    for (const e of list) {
      ...
      const chars = [[...(e.teams[0]?.characters ?? [])], [...(e.teams[1]?.characters ?? [])]] as [
        string[],
        string[],
      ];
      sel.value[e.id] = chars;
      savedChars.value[e.id] = [[...chars[0]], [...chars[1]]];
      const fuses = [e.teams[0]?.fuse ?? null, e.teams[1]?.fuse ?? null] as [
        string | null,
        string | null,
      ];
...
const needsChampions = (e: ManualVideoEntry) =>
  todoOpen.value[e.id] === true ||
  (savedChars.value[e.id] ?? [[], []]).some((side) => side.length === 0);
...
const saveOne = async (e: ManualVideoEntry) => {
  ...
  try {
    ...
    cardNote.value[e.id] = warnings.length
      ? `saved · ${warnings.join('; ')}`
      : 'saved to manual-videos.json';
  } catch (err) {
    cardNote.value[e.id] =
      `save failed: ${(err as { statusMessage?: string }).statusMessage ?? String(err)}`;
  } finally {
    savingId.value = null;
  }
};
