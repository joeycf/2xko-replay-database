<template>
  <section class="mx-auto w-full max-w-[1440px] px-4 py-8 md:px-[26px]">
    <ClientOnly>
      ...
      <h1 class="mt-1 font-display text-d2 font-bold text-ink-primary">Fuse gaps</h1>

      <template v-if="gapsError">
        <p class="mt-6 font-mono text-body text-warning">
          No gap report found — run <span class="text-ink-primary">npm run data:fuse-gaps</span> first.
        </p>
      </template>
      <template v-else-if="gaps">
        <p class="mt-2 font-mono text-[12px] text-ink-muted">
          generated {{ gaps.generatedAt.slice(0, 16).replace('T', ' ') }} · attempt universe {{ gaps.universe.commit }}
          ({{ gaps.universe.videos.toLocaleString() }} ids) · full report in cache/fuse/review/fuse-gaps.md
        </p>

        <!-- summary cards -->
        <div class="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          <div class="cut border border-white/10 bg-surface p-4">
            <div class="font-mono text-data-xl text-ink-primary">{{ gaps.totals.missing }}</div>
            <div class="mt-1 font-sans text-label uppercase text-ink-muted">missing / {{ gaps.totals.videos.toLocaleString() }}</div>
          </div>
          <button
            ...
            :class="bucket === b.id ? 'border-accent bg-surface' : 'border-white/10 bg-surface hover:border-white/25'"
            @click="bucket = bucket === b.id ? 'all' : b.id"
          >
            <div
              class="font-mono text-data-xl"
              :class="bucket === b.id ? 'text-accent' : 'text-ink-primary'"
            >{{ gaps.counts[b.id] }}</div>
            <div class="mt-1 font-sans text-label uppercase text-ink-muted">{{ b.label }}</div>
          </button>
        </div>

        <!-- era filter -->
        <div class="mt-4 flex flex-wrap items-center gap-2">
          <span class="font-mono text-[11px] uppercase text-ink-muted">era</span>
          <button
            v-for="e in ['all', ...eras]"
            :key="e"
            type="button"
            class="border px-2.5 py-1 font-mono text-[11px] uppercase transition-colors"
            :class="era === e ? 'border-accent text-accent' : 'border-white/10 text-ink-secondary hover:border-white/25'"
            @click="era = e"
          >{{ e }}</button>
          <span class="ml-auto font-mono text-[12px] text-ink-muted">{{ filtered.length }} shown</span>
        </div>
        ...
        <div class="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          <div
            ...
            class="flex flex-col"
          >
            <BrowseCard :video="video" />
            <!-- bucket-specific footer -->
            <div class="border border-t-0 border-white/[0.09] bg-[#0C0D12] px-3 py-2 font-mono text-[11px] text-ink-secondary">
              <div class="flex flex-wrap items-center gap-x-3 gap-y-1">
                <span class="uppercase text-ink-muted">{{ gap.bucket }}</span>
                <span>{{ gap.era }}</span>
                <span v-if="gap.bucket === 'unavailable' || gap.bucket === 'pending'">{{ gap.publishedAt.slice(0, 10) }}</span>
                <span
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener"
                  class="ml-auto text-ink-muted underline decoration-white/20 hover:text-ink-primary"
                >youtube ↗</a>
              </div>
              <div
                v-if="gap.detection"
              ...
              >
                <span class="inline-flex items-center gap-1.5">
                  ...
                  />
                  <span v-else>∅</span>
                  <span class="text-ink-muted">d{{ gap.detection.score.left }}</span>
                </span>
                <span class="inline-flex items-center gap-1.5">
                  R
                  <FuseTag
                  ...
                  />
                  <span v-else>∅</span>
                  <span class="text-ink-muted">d{{ gap.detection.score.right }}</span>
                </span>
                <button
                  v-if="gap.frames > 0"
                  type="button"
                  class="ml-auto border border-white/10 px-2 py-0.5 uppercase hover:border-white/25"
                  :class="pillsOpen.has(gap.id) ? 'text-accent' : 'text-ink-secondary'"
              ...
              >
                ...
                >
                  <span class="w-3 flex-none text-ink-muted">{{ side === 'left' ? 'L' : 'R' }}</span>
                  <a
