<template>
  <div class="mx-auto w-full max-w-[1440px]">
    ...
    <div class="relative h-[280px] overflow-hidden border-b border-white/[0.09] md:h-[340px]">
      ...
      <div
        class="absolute inset-0"
        :style="{ background: `radial-gradient(80% 120% at 78% 30%, ${accent}38, transparent 60%), linear-gradient(90deg, #0A0B0F 25%, transparent 70%)` }"
      />
    ...
    <div class="grid grid-cols-1 gap-4 px-4 py-[22px] md:grid-cols-[1.1fr_1fr_1fr] md:px-7">
      ...
      <section class="border border-white/[0.08] bg-[#0F1118] p-5">
        ...
        >
          ...
          >
            <VerifiedMark
              v-if="p.player!.verified"
              :size="10"
            />
            <span class="font-sans text-[13px] font-semibold text-ink-primary">{{ p.player!.displayName }}</span>
            <span class="ml-auto font-mono text-[11px] text-ink-muted">{{ p.count.toLocaleString('en-US') }} played</span>
          </NuxtLink>
        </div>
      </section>
      ...
      <section class="border border-white/[0.08] bg-[#0F1118] p-5">
        ...
        <div class="flex flex-col gap-3.5">
          ...
          <div class="flex items-baseline gap-2">
            <span class="font-display text-[30px] font-bold text-ink-primary">{{ usage.value.toLocaleString('en-US') }}</span>
            <span class="font-sans text-[12px] text-ink-secondary">appearances</span>
          </div>
          <div class="flex items-baseline gap-2">
            <span class="font-display text-[30px] font-bold text-ink-primary">{{ latestCount.toLocaleString('en-US') }}</span>
            <span class="font-sans text-[12px] text-ink-secondary">{{ eraName(latestEra) }} appearances</span>
          </div>
        </div>
      </section>
    </div>
    ...
    <div class="px-4 pb-7 pt-1.5 md:px-7">
      <div class="mb-4 flex items-center gap-2.5">
        ...
        <span class="font-mono text-[12px] text-ink-muted">{{ usage.value.toLocaleString('en-US') }} matches</span>
      </div>
      <ClientOnly>
        <ReplayGrid
          :list="replays"
