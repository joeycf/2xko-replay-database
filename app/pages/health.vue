  ...
  <section class="mx-auto max-w-6xl px-6 py-10">
    ...
    <div class="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
      <div
        v-for="c in cards"
        :key="c.label"
        class="cut border border-white/10 bg-surface p-4"
      >
    ...
    <h2 class="mt-10 font-display text-title font-semibold text-ink-primary">
      Champion portraits
      <span
        class="font-mono text-sub"
        :class="withPortrait === 15 ? 'text-success' : 'text-warning'"
      >
        {{ withPortrait }}/15
      </span>
    </h2>
    <div class="mt-4 grid grid-cols-5 gap-2 sm:grid-cols-8 xl:grid-cols-[repeat(15,minmax(0,1fr))]">
      ...
      >
        ...
        <div
          v-else
          class="cut grid aspect-[3/4] w-full place-items-center bg-elevated font-mono text-[9px] text-ink-faint"
        >
          no art
        </div>
    ...
    <div class="mt-4 flex flex-wrap gap-2">
      ...
      >
        <span
          class="inline-block h-5 w-5 rounded-sm"
          :style="{ background: `var(--champ-${c.id})` }"
        />
...
const cards = computed(() => [{
  label: 'Videos',
  value: videos.value.length.toLocaleString()
}, {
  label: 'Champions',
  value: champions.value.length.toLocaleString()
}, {
  label: 'Players',
  value: players.value.length.toLocaleString()
}, {
  label: 'Verified players',
  value: `${verifiedCount.value} / ${players.value.length}`
}]);
