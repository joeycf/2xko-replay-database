      ...
      <div class="flex flex-col gap-2">
        <div v-for="row in card.rows" :key="row.id">
          <div class="mb-1 flex items-center gap-1.5">
            <span class="h-1.5 w-1.5 flex-none rotate-45" :style="{ background: fuseAccent(row.id) }" />
            <span class="min-w-0 truncate font-sans text-[10.5px] font-semibold text-ink-secondary">{{
              fuseName(row.id)
            }}</span>
