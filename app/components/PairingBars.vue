<template>
  ...
  >
    ...
    >
      ...
      <span class="font-sans text-[13px] font-semibold text-ink-primary">{{ name(p.a) }} + {{ name(p.b) }}</span>
  ...
  >
    ...
    >
      <div class="mb-[5px] flex items-center gap-2">
        <ChampBadge
          :champion-id="mateOf(p)"
          :size="24"
          :notch="6"
          :font-size="10"
        />
        <span class="font-sans text-[13px] font-semibold text-ink-primary">{{ name(mateOf(p)) }}</span>
        <span
          class="count-up ml-auto font-mono text-[11px] text-ink-secondary"
          :data-value="p.value"
        >{{ p.value.toLocaleString('en-US') }}</span>
      </div>
