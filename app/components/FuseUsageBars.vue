<template>
  ...
  >
    ...
    >
      <span class="hidden w-5 flex-none text-right font-mono text-[11px] text-ink-muted sm:block">{{ row.rank }}</span>
      ...
      <span
        class="w-[86px] flex-none font-sans text-[12px] font-semibold text-ink-primary md:w-[104px] md:text-[13px]"
      >{{ row.name }}</span>
      ...
      >{{ row.count.toLocaleString('en-US') }}</span>
    </div>
  </div>
</template>

<script setup lang="ts">
// Ranked fuse team-pick bars (Stats Panel 4, NEW vs design mockups) — same
// anatomy as ChampionUsageBars' dashboard rows with the fuse accent diamond
// in place of a champion badge.
const { detected } = useFuses();

const max = detected[0]?.count || 1;
